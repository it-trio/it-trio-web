  ...
  try {
    ...
    data.segments = data.segments.map(segment => {
      if (segment.speaker && speakerMapping[segment.speaker]) {
        console.log(`  Updating ${segment.speaker} → ${speakerMapping[segment.speaker]}`);
...
function main() {
  ...
  if (args.length < 2) {
    console.error('Usage: node update-speakers.cjs "name1 name2 name3" file1.json file2.json ...');
    process.exit(1);
  }
